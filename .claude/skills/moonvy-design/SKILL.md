...
代码生成 6 步流程必须**严格按顺序执行**。每步有明确的产物（deliverable）和关卡检查（gate check）。

**禁止跳步。** 未完成当前步骤的产物前，不得进入下一步骤。

### 步骤产物与关卡检查

| 步骤 | 必须产物 | 关卡检查 |
|------|---------|---------|
| Step 1 全量图层扫描 | ① 图层树清单（输出到终端） ② 图标清单（单独列表：图层ID、名称、rect、分类） | 图标清单中每个条目都有分类（文字/图片/图标/色块/渐变） |
| Step 2 图标裁剪 | ① 每个 icon 图层裁剪为 `assets/{name}.png` ② 裁剪结果确认（`ls assets/` 输出） | 所有 Step 1 标记为"图标"的图层都有对应的 PNG 文件 |
| Step 3 间距计算 | ① 间距计算表（区块间距、容器padding、子元素间距） | 四边 padding 齐全，所有相邻区块间距已计算 |
| Step 4 图层分类 | ① 每个图层的处理方式映射表 | 无图层被遗漏或分类为"未知" |
| Step 5 代码生成 | ① HTML/CSS 代码文件 ② 自检对照清单 | 所有图层在代码中都有对应实现 |
| Step 6 交互元素 | ① 交互行为列表 + 实现代码 | 按钮/链接/输入都有 hover/focus 状态 |

### 强制行为

- **Step 1 完成后**：必须向用户展示图标清单，数量≥0时进入 Step 2
- **Step 2 完成后**：必须确认裁剪文件存在（`ls` 或文件检查），全部图标处理完毕才进入 Step 3
- **Step 5 完成后**：必须在浏览器中预览并截图对比

## 代码生成关键步骤

> ⚠️ 以下 6 步必须按照「步骤执行协议」严格执行，每步产物未完成前禁止进入下一步。
...
- 导航文字 y=29.49, lineHeight=24, header h=80 → 精确垂直位置应为 29.49px，而非 flex 自动居中
- 下划线 y=59, 文字底边=29.49+24=53.49 → 精确间距 5.5px，实际随意设为 18px

**防范**：
1. Step 3 间距计算必须覆盖**所有可见元素的定位值**，不仅是卡片间距
2. 导航文字的垂直位置 = `text.rect.y - header.rect.y`
3. 激活下划线间距 = `underline.rect.y - (text.rect.y + text.lineHeight)`
4. 禁止依赖 flexbox 自动居中代替精确坐标，除非确认自动居中结果与设计一致

### 图标内嵌文字遗漏（已修复）

**原因**：矢量图标可能包含内嵌文字（如返回顶部按钮的"TOP"文字），这些文字在 JSON 中没有独立的 text 图层，而是烘焙在矢量路径中。跳过图标裁剪后，手绘的 SVG 自然丢失了这些文字。

**防范**：
1. 图标裁剪是唯一可靠的方式获取完整图标内容（包括内嵌文字）
2. 裁剪后检查 PNG 图片内容，确认是否有文字被包含在图标中
3. 如果图标内包含文字，裁剪结果会完整保留，无需额外处理
